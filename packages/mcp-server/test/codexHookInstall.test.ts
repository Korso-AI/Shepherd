...
function expectCanonicalHandlers(source: string, command: string): void {
  const canonical = source.slice(source.indexOf("# Added by Shepherd"));
...
describe("Codex canonical hook installation", () => {
  ...
  it("appends only canonical handlers when hooks is already true", async () => {
    const home = freshHome();
    const existing =
      '[features]\nhooks = true\nweb_search = true\n\n[projects."C:/work"]\ntrust_level = "trusted"\n';
  ...
  it("adds hooks=true inside an existing features table", async () => {
    const home = freshHome();
    writeConfig(
      home,
  ...
  it("appends features and handlers when features is absent", async () => {
    const home = freshHome();
    const existing =
  ...
  it("preserves a no-final-newline config byte-for-byte before its append", async () => {
    ...
    expect(readFileSync(configFile(home), "utf8").startsWith(existing)).toBe(
      true,
    );
  });
  ...
  it("byte-preserves unrelated hook groups", async () => {
    const home = freshHome();
    const existing = [
      "[features]",
      "hooks = true",
  ...
  it("uses a normalized cached Windows path", async () => {
    const home = freshHome();
    const bundle = join(home, "bundled-inboxHook.js");
    writeFileSync(bundle, "// bundled\n", "utf8");

    expect((await install(home, bundle)).status).toBe("installed");
  ...
  it("uses the version-pinned npx fallback", async () => {
    const home = freshHome();
