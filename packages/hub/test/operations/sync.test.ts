...
describe.skipIf(!dbAvailable)(
  ...
  () => {
    ...
    it("#4: sync reports the caller's OWN claims in yourClaims (not in activeClaims)", async () => {
      ...
      expect(aSync.landscape.yourClaims[0]!.intent).toBe("Working on auth module");
    ...
    it("#1 edge-stale: a stale session's claim is hidden from peers until its own sync revives it", async () => {
      ...
      const beforeRecovery = await sync({ sessionId: sessionB.sessionId }, tenant);
      ...
      const afterRecovery = await sync({ sessionId: sessionB.sessionId }, tenant);
    ...
    it("edge-collision: A's sync shows B in conflicts when B later claims overlapping glob", async () => {
      ...
      expect(syncResult.landscape.activeClaims[0]!.intent).toBe("Fix auth file");
