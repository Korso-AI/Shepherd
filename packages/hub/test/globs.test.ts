...
import { normalize, segmentsCompatible, patternsOverlap, globsOverlap } from '../src/globs';
...
describe('globs module', () => {
  ...
  describe('segmentsCompatible()', () => {
    const cases: Array<[string, string, boolean, string]> = [
      ...
      ['*.ts', 'auth.js', true, 'wildcard side -> conservatively compatible (recall)'],
      ['auth.js', '*.ts', true, 'wildcard side -> conservatively compatible (symmetric)'],
  ...
  describe('patternsOverlap()', () => {
    const cases: Array<[string[], string[], boolean, string]> = [
      ...
      [['src', 'auth', '**'], ['src', 'auth', 'login.ts'], true, '** absorbs tail'],
      [['src', 'auth'], ['src', 'payments'], false, 'disjoint literals'],
      [['**', '*.test.ts'], ['src', 'auth', 'foo.test.ts'], true, 'leading ** absorbs prefix'],
    ];
    for (const [a, b, expected, label] of cases) {
  ...
  describe('globsOverlap()', () => {
    ...
    it('any-pair: [src/payments/**, src/auth/x.ts] overlaps [src/auth/**]', () => {
      expect(globsOverlap(['src/payments/**', 'src/auth/x.ts'], ['src/auth/**'])).toBe(true);
    });
    ...
    it('src/auth/login.ts does NOT overlap src/auth/logout.ts', () => {
      expect(globsOverlap(['src/auth/login.ts'], ['src/auth/logout.ts'])).toBe(false);
    });
  });
});
